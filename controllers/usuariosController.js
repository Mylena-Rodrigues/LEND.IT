...
const usuariosControllers = {
  //Listar usuários
  index: async (req, res) => {
    const listUsuarios = await Usuarios()
      .findAll()
      .then((listUsuarios) => {
  ...
  create: async (req, res) => {
    ...
    const novoUsuario = await Usuarios()
      .create({ email: email, nome: name, senha: senhaEncript })
      .then((novoUsuario) => {
  ...
  update: async (req, res) => {
    ...
    console.log(id, name);
    const modUser = await Usuarios().update(
      { nome: name, email, senha },
      { where: { id } }
    );
  ...
  //   const modUsuario = await Usuarios()
  //     .update({ email }, { where: { id: id } })
  //     .then((modUsuario) => {
  ...
  updatePassword: async (req, res) => {
    ...
    const modUsuario = await Usuarios()
      .update({ senha: senhaEncript }, { where: { id } })
      .then(() => {
  ...
  delete: async (req, res) => {
    const { id } = req.params;
    const delUsuario = await Usuarios()
      .destroy({ where: { id } })
      .then((delUsuario) => {
